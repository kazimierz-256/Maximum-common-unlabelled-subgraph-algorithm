...
namespace SubgraphIsomorphismBenchmark
{
    class Program
    {
        ...
        private static TimeSpan BenchmarkIsomorphism(int n, double density, int seed)
        {
            ...
            // run the algorithm
            sw.Start();
            SubgraphIsomorphismExactAlgorithm.ParallelSubgraphIsomorphismExtractor<double>.ExtractOptimalSubgraph(g, h, (v, e) => v, 0, true, out double score, out var gToH, out var hToG);
            sw.Stop();
            return sw.Elapsed;
        }
    }
